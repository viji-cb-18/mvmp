...
import { getCategory, addParentCategory, addSubCategory, deleteCategory, updateCategory } from '../../services/categoryServices';
import { toast } from 'react-toastify';

const CategoryPage = () => {
  const [categories, setCategories] = useState([]);
  const [formData, setFormData] = useState({ name: '', parentCategory: '', image: null });
  const [isSub, setIsSub] = useState(false); // toggle between parent or subcategory
  ...
  const handleEditCategory = async (id) => {
    // Add logic for editing category
    // For now, just log the category ID
    console.log(`Edit category with ID: ${id}`);
    // You can show a form pre-filled with the category's data here
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <form
        className="bg-white p-4 rounded shadow-md mb-6 flex flex-col gap-4 max-w-xl"
      ...
      >
        ...
        >
          Add {isSub ? 'Subcategory' : 'Parent Category'}
        </button>
      </form>
      ...
      <div className="bg-white p-4 rounded shadow">
        ...
        {categories.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-4">
            {categories.map((cat) => (
              <div key={cat._id} className="border p-3 rounded flex flex-col items-center">
                {cat.image && (
                ...
                {cat.subcategories?.length > 0 && (
                  <ul className="ml-4 mt-2 list-disc text-sm text-gray-600 space-y-1">
...
const CategoryPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = new FormData();
    payload.append('name', formData.name);
    if (formData.image) payload.append('image', formData.image); // ✅ must match backend
    if (isSub) payload.append('parentCategory', formData.parentCategory);
    ...
    try {
      if (editMode) {
        ...
        if (isSub) {
          ...
          await addParentCategory(payload); // 👈 this calls the route POST /api/categories/parent
          toast.success('Parent category added');
        }
      }
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold">
          {editMode ? 'Edit Category' : 'Category Management'}
        </h2>
      </div>
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          >
            {editMode ? 'Update Category' : `Add ${isSub ? 'Subcategory' : 'Parent Category'}`}
          </button>
